...
-- Temporary table for materialized view
CREATE MATERIALIZED VIEW last_version_aggregate_view AS
SELECT DISTINCT ON (e.id, e.type)
    e.id,
    e.type,
    e.stream_name,
    e.version,
    ...
    e.metadata,
    e.content,
    e.content->>'customer_id' AS customer_id,
    e.content->>'order_id' AS order_id
FROM stream_event e
ORDER BY e.id, e.type, e.version DESC;

-- Add indexes on customer_id, order_id, and create a unique index on (id, type, version)
CREATE INDEX idx_mv_customer_id ON last_version_aggregate_view(customer_id);
CREATE INDEX idx_mv_order_id ON last_version_aggregate_view(order_id);
CREATE UNIQUE INDEX uk_mv ON last_version_aggregate_view(id, type, version);

CREATE OR REPLACE FUNCTION refresh_last_version_aggregate_view() RETURNS TRIGGER AS $$
BEGIN
    -- Update the materialized view with the latest versions
    REFRESH MATERIALIZED VIEW CONCURRENTLY last_version_aggregate_view;

    RETURN NULL;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER refresh_last_version_aggregate_trigger
    AFTER INSERT ON stream_event
    FOR EACH STATEMENT EXECUTE FUNCTION refresh_last_version_aggregate_view();
